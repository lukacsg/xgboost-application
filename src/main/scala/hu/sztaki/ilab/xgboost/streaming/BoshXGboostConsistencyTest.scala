...
object BoshXGboostConsistencyTest {
  val logger = LogFactory.getLog(this.getClass)


  def readTestLibSVM(env: StreamExecutionEnvironment, filePath: String, dimension: Int, numberOfParallelism: Int,
                     skipBrokenLine: Boolean = false): DataStream[(LabeledVector, Int)] = {
    env.readTextFile(filePath).setParallelism(numberOfParallelism)
      .flatMap {
        new RichFlatMapFunction[String, (Double, Array[(Int, Double)], Int)] {


          override def flatMap(line: String, out: Collector[(Double, Array[(Int, Double)], Int)]): Unit = {
    ...
    parameterCheck(args).foreach(propsPath => {
      ...
      if (act == Action.Predict) {
        ...
        XGboost.predictWithId(
          ...
          numberOfParallelism)
          .addSink(
            new RichSinkFunction[Array[(Array[Float], Int)]] {
              import scala.collection.mutable._
              val wholeModel: ArrayBuffer[(Array[Float], Int)] = ArrayBuffer.empty[(Array[Float], Int)]
              val treshold = 0.5

              override def invoke(value: Array[(Array[Float], Int)]) {
                wholeModel ++= value
              }
